...
protected:
    ...
    void SetUp() {
        ...
        repo_nuestro = {'l', 'f', 'f', 'o', 'w', 'g', 'a', 'v', 'h', 'c', 'c', 'a', 'e', 't', 'g', 'w', 'r', 'e', 'a', 'a', 'f', 'e', 't', 'g', 'w', 'r', 'e', 'a', 'a', 'f', 'e', 't', 'g', 'w', 'r', 'e'};
...
//Test Agregado
TEST_F(ServidorTest, jugador_1_ubica_cuando_empieza_esta_bien) {
    Nat cantJugadores = 3;
    Nat tamanoTab = 10;
    Nat cantFichas = 3;

    Fachada_Variante var(tamanoTab, cantFichas, puntajes, palabrasLegitimas);
    Fachada_Servidor f(cantJugadores, var, repo_nuestro);
    ...
    compareNotificaciones(f.notificaciones(id1), {
            ...
            Notificacion::nuevaEmpezar(tamanoTab),
            Notificacion::nuevaTurnoDe(id1),
            Notificacion::nuevaReponer({ 'l', 'f', 'f'})
    });
    ...
    compareNotificaciones(f.notificaciones(id2), {
            ...
            Notificacion::nuevaEmpezar(tamanoTab),
            Notificacion::nuevaTurnoDe(id1),
            Notificacion::nuevaReponer({ 'o', 'w', 'g'})
    });
    ...
    compareNotificaciones(f.notificaciones(id3), {
            ...
            Notificacion::nuevaEmpezar(tamanoTab),
            Notificacion::nuevaTurnoDe(id1),
            Notificacion::nuevaReponer({ 'a', 'v', 'h'})
    });

    f.notificaciones(id1);
    f.recibirMensaje(id1, {{4,4, 'f'}});

    compareNotificaciones(f.notificaciones(id1), {
            Notificacion::nuevaUbicar(id1, {{4,4, 'f'}}),
            Notificacion::nuevaSumaPuntos(id1,2),
            Notificacion::nuevaReponer({'c'}),
            Notificacion::nuevaTurnoDe(id2)
    });
};
